...
import type { AuditSession, AuditCategory, AuditCriterion, AuditResponse } from '../../../types';
...
export function SEAConductPage() {
  ...
  const handlePhotoRemove = async (criterionId: string) => {
    try {
      ...
      console.error('Foto entfernen fehlgeschlagen:', error);
    }
  };
  ...
  const handleComplete = async () => {
    setCompleting(true);
    try {
      await api(`/api/tools/sea/sessions/${id}/complete`, { method: 'POST' });
      navigate(`/tools/sea/sessions/${id}`);
    } catch (error) {
      console.error('Audit abschliessen fehlgeschlagen:', error);
      setCompleting(false);
    }
  };

  if (loading || !session) {
    return <LoadingSpinner text={t.audit.loading} />;
  }
  ...
  return (
    <div className="flex flex-col min-h-[calc(100dvh-52px-72px)]">
      {/* Sticky Progress + Category Tabs */}
      <div className="sticky top-[52px] z-10 bg-kore-white border-b border-kore-border px-lg py-md-sm">
        <div className="flex items-center justify-between mb-sm">
          <span className="text-caption text-kore-mid uppercase tracking-widest truncate">
            {session.template?.name}
          </span>
          <span className="text-small text-kore-mid whitespace-nowrap">
            {answeredCriteria}/{totalCriteria} {t.audit.rated}
          </span>
        </div>
        ...
        <div className="w-full h-1.5 bg-kore-border rounded-full overflow-hidden">
          <div
            className="h-full bg-kore-brass transition-all duration-300 rounded-full"
            style={{ width: `${progressPct}%` }}
          />
        </div>

        {/* Category Tabs — horizontally scrollable */}
        <div className="flex gap-sm mt-md-sm overflow-x-auto -mx-lg px-lg scrollbar-hide">
          {categories.map((cat, idx) => (
            <button
              key={cat.id}
              onClick={() => setCurrentCatIdx(idx)}
              className={`whitespace-nowrap px-md-sm py-sm text-caption uppercase tracking-widest transition-colors touch-manipulation min-h-[36px] ${
                idx === currentCatIdx
      ...
      {/* Criteria List */}
      <div className="flex-1 px-lg py-lg space-y-lg">
            ...
            >
              ...
              <div className="flex items-start justify-between gap-md">
                ...
                {criterion.isRequired && (
                  <span className="text-caption text-kore-brass shrink-0">{t.audit.required}</span>
                )}
              </div>
              ...
              <div>
                <label className="label-default">
                  {t.audit.score}: {state.scorePercent ?? '—'}%
                </label>
                <input
                  type="range"
                  min={0}
              ...
              <div className="flex items-end gap-xl">
                <div>
                  <label className="label-default">{t.audit.pass}?</label>
                  <PassFailToggle
                    value={state.passed}
                    onChange={(val) => {
                      setResponses((prev) => {
                ...
                <div>
                  <label className="label-default">{t.audit.photo}</label>
                  <PhotoUploader
                    photoUrl={
                      state.photoPath
              ...
              {/* Kommentar */}
              <div>
                <label className="label-default">{t.audit.comment}</label>
                <textarea
                  value={state.comment}
                  onChange={(e) => {
                    setResponses((prev) => {
              ...
              {state.saving && (
                <span className="text-caption text-kore-faint">{t.audit.saving}</span>
              )}
            </div>
          );
        })}
      ...
      <div className="sticky bottom-[56px] bg-kore-white border-t border-kore-border px-lg py-md flex items-center justify-between z-10">
        <button
          onClick={() => setCurrentCatIdx((i) => Math.max(0, i - 1))}
          disabled={isFirstCategory}
          className="flex items-center gap-xs text-small text-kore-mid hover:text-kore-ink disabled:opacity-30 transition-colors touch-manipulation min-h-[48px]"
        >
          <ChevronLeft size={18} />
          {t.audit.previous}
        </button>
        ...
        {isLastCategory ? (
          <button
            ...
            disabled={completing}
            className="flex items-center gap-sm bg-kore-success text-white px-xl py-md-sm text-small font-medium uppercase tracking-widest hover:opacity-90 transition-opacity disabled:opacity-50 touch-manipulation min-h-[48px]"
          >
            <CheckCircle size={18} />
            {completing ? t.audit.completing : t.audit.complete}
          </button>
        ) : (
          <button
            onClick={() => setCurrentCatIdx((i) => Math.min(categories.length - 1, i + 1))}
            className="flex items-center gap-xs text-small text-kore-brass hover:text-kore-brass-dk transition-colors touch-manipulation min-h-[48px]"
          >
            {t.audit.next}
